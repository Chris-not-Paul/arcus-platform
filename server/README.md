...
`POST /api/professional/hazard-exposure/point` evaluates official point-level hazard exposure in shadow mode. The first provider queries ISPRA WFS hydraulic layers `nz1:aree_peric_idraulica_p1`, `nz1:aree_peric_idraulica_p2` and `nz1:aree_peric_idraulica_p3` from the backend, keeps WMS strictly visual and returns source status separately from current ARCUS historical proxy scores.
