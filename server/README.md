...
- `GET /api/open/manifest`
- `GET /api/open/taxonomy`
- `GET /api/open/data-dictionary`
- `GET /api/open/changelog`
- `GET /api/open/statistics`
- `GET /api/open/quality-audit`
- `GET /api/open/id-mapping`
- `GET /api/open/download/csv`
- `GET /api/open/download/geojson`
...
The hydraulic response preserves the backward-compatible provider `status` and adds an explicit completeness contract: `assessment_complete`, `decision_status` and `coverage.failed_layers`. `decision_status` distinguishes `available_complete`, `available_partial`, `no_intersection` and `source_incomplete`. Each layer is evaluated by ISPRA GeoServer through an exact `INTERSECTS(geom,SRID=4326;POINT(longitude latitude))` predicate; ARCUS requests only the corresponding `scenariopN` attribute, avoiding transfer of the source polygon geometry. P1/P2/P3 successes are cached independently for six hours, concurrent duplicate requests are coalesced and transient timeouts/network/502/503/504 failures receive one bounded retry. Cached results retain per-layer cache metadata and are never represented as a new live observation. Run `npm run test:hazard:hydraulic:live` for the canonical live P1, P2, P3 and Torino readiness gate, including the 5 KB per-layer payload ceiling.

Hydraulic v1.2 also persists complete point/layer observations under `private-data/hazard/hydraulic-observations` (override with `ARCUS_HYDRAULIC_OBSERVATION_DIR`). A persisted observation is usable only inside the six-hour freshness window and is labelled `persistent_cache`; older observations may be exposed only as `last_known_good` context while the current decision remains `source_incomplete`. Remote calls use a six-request concurrency bulkhead, bounded retry with jitter and a per-layer circuit breaker that opens after three consecutive technical failures for 30 seconds. `/api/admin/ops/status` and `/api/admin/metrics` expose observation-store reachability, count and age. The live gate verifies all three `DescribeFeatureType` contracts before testing the canonical points. This point store is not a national ISPRA dataset mirror and cannot resolve a previously unseen point during a total upstream outage.

Landslide v2 uses the same conservative completeness model. It sends an exact
server-side `INTERSECTS` point query and requests only `cod_per_it`, avoiding
PAI polygon downloads while preserving all classes returned at the point.
Complete observations are cached in memory and under
`private-data/hazard/landslide-observations` (override with
`ARCUS_LANDSLIDE_OBSERVATION_DIR`). Current persistent observations are usable
for six hours; older observations are context-only last-known-good data for up
to 30 days. Retry, in-flight deduplication, a six-request bulkhead and a
three-failure/30-second circuit breaker protect the upstream service. Ops
status and metrics expose the landslide store independently. This store is
also point-based, not a national ISPRA mirror.
