...
async function pruneObservationStoreNow(directory) {
  ...
  const overflow = retained.slice(STORE_MAX_ITEMS);

  await Promise.all(
    overflow.map((item) => fs.rm(item.filePath, {
      force: true,
    }))
  );
}

export async function readHydraulicObservation({
  cacheKey,
  directory = hydraulicObservationDir,
  now = Date.now(),
} = {}) {
  if (!cacheKey) {
    return null;
  ...
  try {
    ...
    if (["ENOENT", "EISDIR", "SyntaxError"].includes(error?.code)) {
      return null;
    }
...
export async function writeHydraulicObservation({
  cacheKey,
  directory = hydraulicObservationDir,
  observedAt,
  result,
...
export async function hydraulicObservationStoreStatus({
  directory = hydraulicObservationDir,
} = {}) {
  try {
    const entries = await fs.readdir(directory, {
    ...
    for (const entry of files) {
      try {
        ...
        // A corrupt observation is ignored and will never be used as fallback.
      }
    }
...
export const HYDRAULIC_OBSERVATION_STORE_SCHEMA_VERSION =
